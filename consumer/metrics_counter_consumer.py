...
class MetricsCounterConsumer(object):
    ...
    def consume_alarm_with_notry(self):
        while True:
            ...
            if alarm_payload_list:
                ...
                slog.info("try_store_cache")
                self.metrics_counter_try_store_cache()
        return
    ...
    def metrics_counter_try_store_cache(self):
        for _db, _cache in self.counter_insert_cache.items():
            if len(_cache) > 0:
                self.mysql_db.multi_insert_into_db(_db, "metrics_counter", _cache)
                self.counter_insert_cache[_db] = []
