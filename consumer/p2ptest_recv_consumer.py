...
class P2PTestRecvConsumer(object):
    def __init__(self, q, queue_key_list, alarm_env='test'):
        slog.info("P2PTestRecvConsumer init. pid:{0} paraent:{1} queue_key:{2}".format(
    ...
    def consume_alarm_with_notry(self):
        while True:
            ...
            for alarm_payload in alarm_payload_list:
                ...
                if alarm_type == 'p2ptest_recv_info':
                    slog.info(alarm_payload.get('packet'))
                    self.p2ptest_recv_handle(alarm_payload.get('packet'))
                else:
    ...
    def p2ptest_recv_handle(self, packet):
        slog.info(packet)
        '''
        {
            "alarm_type": "p2ptest_recv_info",
            "packet": {
